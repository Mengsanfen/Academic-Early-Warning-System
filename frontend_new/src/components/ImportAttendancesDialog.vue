    ...
    <div class="import-content">
      ...
      <div v-show="currentStep === 0" class="upload-section">
        <div class="template-download">
          <span>请先下载导入模板，按照模板格式填写考勤数据</span>
        ...
        <div class="import-options">
          <el-form label-width="120px">
            <el-form-item label="更新已存在记录">
              <el-switch v-model="updateExisting" />
              <span class="option-hint">开启后将更新已存在的考勤记录</span>
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="status-hint">
          <el-alert type="info" :closable="false">
            <template #title>
              考勤状态说明：正常(present)、迟到(late)、旷课(absent)、请假(leave)
            </template>
          </el-alert>
        </div>
      </div>
...
import { downloadAttendanceTemplate, importAttendances } from '@/api/import'
...
const handleImport = async () => {
  ...
  try {
    const result = await importAttendances(selectedFile.value, updateExisting.value)
