...
你是一个顶尖架构开发师，我现在系统有一些现存问题，部分功能没有实现，请你帮我进行开发完善功能：
1. 考虑到高校实际可能各个年级制度规则等可能不一致，并不是某套规则一定适用于所有年级，所以请你帮我开发规则权限，在规则配置http://localhost:3000/rules这里，可以选择实施对象，每一条规则可以选择执行的年级、班级等。

2. 配置里面学分分类，如选修或者必修，不同的类型不同要求。因为实际高校是选修课学分和必修课学分的要求不一样，考虑到实际应用，系统中应该对于课程管理这块，进行预设好必修和选修的属性，并且可以进行修改配置，在规则执行的时候就可以新增规则符合模拟高校实际应用，按照选修学分要求和必修学分要求不同来进行制定规则了，这里请你考虑到实际高校应用，帮我最大化地去开发接近高校应用的功能模块，并检查系统整体保证全流程适配畅通。